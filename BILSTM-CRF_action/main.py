...
def evaluate(sess, model, name, manager, id_to_tag, logger):
    ...
    for line in eval_lines:
        logger.info(line)  #将模型评测的结果，f1,pre,recall存进日志中
...
def train():
    # 加载数据集
    train_sentences = data_load.load_sentences(FLAGS.train_file)
    dev_sentences = data_load.load_sentences(FLAGS.dev_file)
    test_sentences = data_load.load_sentences(FLAGS.test_file)

    # 转换编码 bio转bioes
    data_load.update_tag_scheme(train_sentences, FLAGS.tag_schema)
    data_load.update_tag_scheme(test_sentences, FLAGS.tag_schema)
    data_load.update_tag_scheme(dev_sentences, FLAGS.tag_schema)

    # 创建单词映射及标签映射
    if not os.path.isfile(FLAGS.map_file):
        if FLAGS.pre_emb:
            dico_words_train = data_load.word_mapping(train_sentences)[0]
            ...
            _, word_to_id, id_to_word = data_load.word_mapping(train_sentences)

        _, tag_to_id, id_to_tag = data_load.tag_mapping(train_sentences)
    ...
    #把整体数据加入其中
    train_data = data_load.prepare_dataset(
        train_sentences, word_to_id, tag_to_id
    )

    dev_data = data_load.prepare_dataset(
        dev_sentences, word_to_id, tag_to_id
    )

    test_data = data_load.prepare_dataset(
